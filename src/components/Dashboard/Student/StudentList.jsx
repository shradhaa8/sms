...
useEffect(()=> {
        ...
        const data = await response.data.students.map((std) =>(
          {
            ...
            image: <img width={40} className='rounded-full' src={`http://localhost:5000/${std.image}`} />,
  ...
  const handleFilter = () =>{
    const records = students.filter((std) =>(
      std.s_name.toLowerCase().includes(e.target.value.toLowerCase())
    ))
  ...
  return (
    ...
    <div className='flex justify-between items-center'>
        <input
            type='text' placeholder='Search Student' onChange={handleFilter} className='px-4 py-0.5 border' 
        />
